...
class RedisClient
{
    ...
    public function connect($host = '127.0.0.1', $port = 6379, $silent_fail = false, $timeout = 60)
    {
        ...
        if ($silent_fail) {
            ...
            $this->handle = fsockopen($host, $port, $errno, $errstr, $this->connect_timeout);
            if(!$this->handle) {die("Can not connect to redis");}
        }
    ...
    public function Existed($key)
    {
     ...
     /*beginregion custom function */
    /**
     * return 1 if can get lock
     *if(LockTake("key","val",30)==1){  try { do_IfGetLock_OK() } finnally{ LockRelease("key","val") ;} }
     * @param [type] $key
     * @param [type] $stringValue
     * @param [type] $expireInSeconds
     * @return void
     */
    public function LockTake($key, $stringValue,  $expireInSeconds)
    {
        $ok =  $this->cmd("SETNX", $key, $stringValue)->set();
        $this->cmd("EXPIRE", $key, $expireInSeconds)->set();
        return $ok[0] ;
    }

    public function LockRelease($key, $stringValue)
    {
        $owner = $this->cmd("GET", $key)->get();;
        if ($owner == $stringValue) {
            $ok = $this->cmd('DEL', $key)->set();
            return $ok[0];
        }
